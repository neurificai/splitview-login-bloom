...
const ContactBox: React.FC<ContactBoxProps> = ({ contacts }) => {
  return (
    <Card className="shadow-md border-none bg-white transition-all">
      <CardContent className="p-3">
        <div className="space-y-2">
          {contacts.slice(0, 2).map((contact, index) => (
            <div key={index} className="flex items-center gap-2 p-2 rounded-lg hover:bg-sky-50 transition-colors">
          ...
          {contacts.length > 2 && (
            <div className="text-center mt-1">
              <a href="#" className="text-xs text-blue-500 flex items-center justify-center gap-1 hover:underline">
